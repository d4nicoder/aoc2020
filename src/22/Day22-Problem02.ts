...
type IMaze = Map<string, any>

// NEW METHOD
let Game = 0
let Round = 0
let SubRounds: {[game: number]: number} = {}
...
const playRound = (player1: number[], player2: number[], gameMaze: IMaze): number => {
...
let winDeck: number[] = []
const playGame = (player1: number[], player2: number[]): number => {
    Game++
    log(`\n====== GAME ${Game} ======\n`)
...
const result = (res: number[]): number => {
...
const main = async (): Promise<number> => {
    const lines = await readLines(path.join(__dirname, INPUT))
    let player: number = 0
    lines.forEach((line) => {
        if (line === 'Player 1:') {
            player = 1
